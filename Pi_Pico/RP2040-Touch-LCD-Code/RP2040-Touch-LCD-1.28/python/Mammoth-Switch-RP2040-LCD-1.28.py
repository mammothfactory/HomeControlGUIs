...
SCK = 10        # SPI protocal - Serial Clock pin
MOSI = 11       # SPI protocal - Master Output Slave Input pin
MISO = 12       # SPI protocal - Master In Slave Output pin
RST = 13        # Reset pin
...
def transition_screens(switchControllerId: int, trigger: hex, source: int, state: list) -> int:
    """ Determine which screen to display next 

    Args:
        switchControllerId (int): ID type for switch to control 
        trigger (hex): A Touch.Gestures input by users finger (e.g. Light swipe = 0x03)
        source (int): Current state machine screen being displayed
        states (list): Full list of possible on screen states
    Returns:
        int: Index of states List to transition display to
    """
    UP_SWIPE = 0x01 
    DOWN_SWIPE = 0x02
    LEFT_SWIPE = 0x03
    ...
    # Polar cordinates for pie chart like areas acting like buttons on touchscreen
    # https://en.wikipedia.org/wiki/Polar_coordinate_system
    LIGHT_ICON_CLICK = (211, 330)
    FAN_ICON_CLICK = (91, 210)
    LOUVRE_ICON_CLICK = (691, 810)  # minus (360, 360) = (331, 90)


    if switchControllerId == 0:
        # Create screens for lights, fans, and louvres
        screenTrasitions = [
            {'trigger': LEFT_SWIPE, 'source': '', 'destination': ''},
            {'trigger': '', 'source': '', 'destination': ''},
            {'trigger': '', 'source': '', 'destination': ''},
            {'trigger': '', 'source': '', 'destination': ''},
            
            {'trigger': 'start', 'source': state[0], 'destination': state[1]},
        ]
        
    elif switchControllerId == 1 or switchControllerId == 2:
        # Create screen for one light in closet or two-way light in hallway
                screenTrasitions = [
            {'trigger': '', 'source': '', 'destination': ''},
            {'trigger': '', 'source': '', 'destination': ''},
            {'trigger': '', 'source': '', 'destination': ''},
        ...
        print(f'{switchControllerId} is an INVALID Switch Controller ID')
        
    return destination


def convert_cartesian_to_polar(x: int, y: int) -> tuple:
    r = math.sqrt(x**2 + y**2)
...
def convert_polar_to_cartesian(angleInDegrees: int, radiusInPixels: int=120)-> tuple:
    """"
    # Screen is 240 x 240 square
...
def is_within_polar_sweep(xTouchInput, yTouchInputnput, iconPolarLocation: tuple) -> bool:
    _, thetaInDegrees = convert_cartesian_to_polar(xTouchInput, yTouchInputnput)

    startOfSweep, endOfSweep = (element % 360 for element in iconPolarLocation)
    
    if startOfSweep <= thetaInDegrees <= endOfSweep:
        return True
    else: 
        return False


def configure_server(switchControllerId: int):
    pass
...
if __name__=='__main__':
    ...
    resetToHomeTimeoutInSeconds = 5.0
    startTimeInSeconds = time.time()
    
    SCREEN_STATE_MACHINE = ['Home', 'TripleIconHome', 'SingleIconHome', 'LightLevel1', 'LightLevel2', 'LightLevel3', 'FanLevel1', 'FanLevel2', 'FanLevel3', 'LouvreOpen', 'LouvreClosed']
    
    
    while(True):
        if startTimeInSeconds % resetToHomeTimeoutInSeconds == 0:
            DisplayHome   
